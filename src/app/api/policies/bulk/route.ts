...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: `${result.count} policy(ies) deleted successfully`,
      count: result.count,
    });
  } catch (error) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: `${result.count} policy(ies) updated successfully`,
      count: result.count,
    });
  } catch (error) {
    console.error('Bulk update policies error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
